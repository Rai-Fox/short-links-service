...
class LinksRepository(BaseRepository):
    ...
    async def delete_link(self, short_code: str, delete_by: str) -> bool:
        ...
        self.redis_client.delete(short_code)
    ...
    def get_original_link_from_cache(self, short_code: str) -> LinkInDB | None:
        ...
        cached_link_json = self.redis_client.get(short_code)
    ...
    def set_link_to_cache(self, short_code: str, link: LinkInDB) -> None:
        ...
        self.redis_client.set(short_code, link_json, ex=ttl)
        logger.info(f"Link set to cache for {short_code} with TTL: {ttl} seconds")


def get_links_repository(
    session: AsyncSession = Depends(get_async_db_session), redis_client: Redis = Depends(get_redis_client)
) -> LinksRepository:
    """FastAPI dependency provider for LinksRepository."""
    return LinksRepository(session=session, redis_client=redis_client)
    ...
    while True:
        try:
            async with async_session_factory() as session:
                links_repository = LinksRepository(session=session, redis_client=get_redis_client())
